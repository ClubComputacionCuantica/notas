Una _variedad diferenciable_ es un [espacio topológico](../Topología/Espacio%20topológico.md) $M$ que localmente se parece al espacio euclídeo $\mathbb{R}^n$, y donde las transiciones entre coordenadas son funciones diferenciables. Más precisamente, una variedad diferenciable de dimensión $n$ es un par $(M, \mathcal{A})$ donde:

- $M$ es un [espacio topológico](../Topología/Espacio%20topológico.md) de Hausdorff y [segundo axiomático numerable](Segundo%20axiomático%20numerable),
- $\mathcal{A}$ es un [atlas diferenciable](Atlas%20diferenciable): una colección de pares $(U_i, \varphi_i)$ llamados _cartas_, donde:
