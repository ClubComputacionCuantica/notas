Sea $V$ un [espacio vectorial](Espacio%20vectorial.md) de [dimensión](Base%20y%20dimensión.md) $n$ sobre un campo $\mathbb{F}$, y sean $\mathcal{B} = \{ \mathbf{v}_1, \ldots, \mathbf{v}_n \}$ y $\mathcal{B}' = \{ \mathbf{w}_1, \ldots, \mathbf{w}_n \}$ dos [bases](Base%20y%20dimensión.md) de $V$. Es posible convertir las coordenadas de un vector en una base $\mathcal{B}$ a sus coordenadas en otra base $\mathcal{B}'$.
### Matriz de cambio de base
Existe una [matriz invertible](../Matrices/Matrices.md) $P$ tal que, para cualquier vector $\mathbf{v} \in V$ con coordenadas $[v]_{\mathcal{B}}$ en la base $\mathcal{B}$ y $[v]_{\mathcal{B}'}$ en la base $\mathcal{B}'$, se cumple:
$$
...
3. Si $A$ es la [](../Operadores%20lineales/Operadores%20lineales.md#Representación%20matricial|matriz) de un operador lineal $T$ en la base $\mathcal{B}$, y $A'$ su matriz en la base $\mathcal{B}'$, entonces:$$
   A' = P A P^{-1}
   $$
