...
AA^{-1} = A^{-1}A = I,
$$
donde $I$ es la [[matriz identidad]] de orden $n$. A dicha matriz $A^{-1}$ se le llama la _inversa_ de $A$.

## Matrices singulares
Si $A$ no es invertible se dice que es *singular*.
### Caracterización
Una matriz $A$ es invertible si y solo si:
- Su [[determinante]] es distinto de cero: $\det(A) \neq 0$.
- El sistema lineal $A \mathbf{x} = \mathbf{b}$ tiene solución única para toda $\mathbf{b} \in \mathbb{F}^n$.
- Las columnas y filas de $A$ forman una [[Base y dimensión|base]] de $\mathbb{F}^n$.
- ...
### Propiedades
1. $(A^{-1})^{-1} = A$
2. $(AB)^{-1} = B^{-1} A^{-1}$
